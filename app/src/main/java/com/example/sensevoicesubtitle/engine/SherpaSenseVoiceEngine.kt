...
class SherpaSenseVoiceEngine(context: Context) : SenseVoiceEngine {
    ...
    /** A subtitle-friendly grouping heuristic. SenseVoice returns token timestamps; we create readable cues at punctuation. */
    private fun groupText(text: String, tokenStarts: List<Long>, offsetMs: Long, durationMs: Long): List<SubtitleCue> {
        val chars = text.toList()
        val n = max(chars.size, 1)
        val tokenStart = tokenStarts.firstOrNull() ?: offsetMs
        val tokenEnd = tokenStarts.lastOrNull()?.plus(900) ?: offsetMs + durationMs
        val end = max(offsetMs + 1200, tokenEnd).coerceAtMost(offsetMs + durationMs + 1500)
        val boundaries = chars.indices.filter { chars[it] in "。！？!?；;\n." }
        if (boundaries.isEmpty()) return listOf(SubtitleCue(tokenStart, end, text))
        val cues = mutableListOf<SubtitleCue>(); var last = 0; var cueStart = tokenStart
        for (b in boundaries) {
            val fragment = chars.subList(last, b + 1).joinToString("").trim(); if (fragment.isBlank()) { last = b + 1; continue }
            val progress = (b + 1).toDouble() / n; val cueEnd = (tokenStart + ((end - tokenStart) * progress)).toLong()
            cues += SubtitleCue(cueStart, max(cueStart + 700, cueEnd), fragment); cueStart = max(cueStart + 700, cueEnd); last = b + 1
        }
        if (last < chars.size) cues += SubtitleCue(cueStart, end, chars.subList(last, chars.size).joinToString("").trim())
        return cues
    }
