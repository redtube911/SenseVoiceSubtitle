...
import com.example.sensevoicesubtitle.util.AssWriter
    ...
    fun start() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                baseDir.mkdirs()
                val baseName = safeName(videoName.value)
                val srt = File(baseDir, "$baseName.srt")
                val ass = File(baseDir, "$baseName.ass")
                srt.writeText(SrtWriter.format(cues, settings.value.showEmotion, settings.value.showEvent), Charsets.UTF_8)
                ass.writeText(AssWriter.format(cues, settings.value.showEmotion, settings.value.showEvent), Charsets.UTF_8)
                // ASS explicitly pins subtitles to bottom-center; SRT remains available for compatibility.
                output.value = ass
                _progress.value = ProgressState("完成", 100, 100, "字幕生成完成，共 ${cues.size} 条；已同时生成 SRT + ASS（ASS 固定底部）")
